...
def critical_point_type(x,y,Fx,Fy,ind_adj):
    ...
    cr_type = np.zeros(len(ind_adj[0]),dtype=np.uint32) + 6
...
def stream_lines(x,y,Fx,Fy,ind_adj):
    ...
    F =  np.sqrt(Fx**2 + Fy**2)
    text_map = ['Repelling Node', 'Attracting Node', 'Saddle Node', 'Repelling Focus','Attracting Focus', 'Center','error']
    color_map = ['#ff756d', '#5c9090', '#e9b666' , '#f5b19c', '#0f4c81','#328729','ff0000']
    ...
    for ki in range(len(ind_adj[0])):
        ...
        # ********************************************************************************
        # This del_t value determines how far way from the critical point the separatrices 
        # are drawn. For CA, del_t = 1 is taken. For test datasets, del_t = 0.1 is used.
        # ********************************************************************************
        ...
        py.streamplot(x, y, Fx, Fy, color='k', linewidth=1, start_points=seed_points.T)
        ...
        py.plot(xc,yc,'o',color = color_map[c_type[ki]],markeredgecolor='k',markersize=12)
